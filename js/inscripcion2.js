...
function validar() {
    ...
    let nombreApellido = document.getElementById("nombreyapellido1").value;
    if (!regexLetras.test(nombreApellido) || (nombreApellido.length == 0 || nombreApellido.length > 10)) {
    ...
    if (errorDni) {
        document.getElementById("errorDni").innerHTML = mensajeDni;
    }
...
totalAgregado.innerHTML = Number(15000);
...
agregarPersona.addEventListener("click", () => {
    ...
    let nuevoImporte = importeActual + 15000;
...
for (let i = 1; i <= personasAgregadas.length; i++) {
    ...
    if (errorNombreApellido) {
      ...
      document.getElementById(`errorNombreApellido${i}`).innerHTML = ""; // Limpiar el mensaje de error anterior
    }
    ...
    if (errorEmail) {
      ...
      document.getElementById(`errorEmail${i}`).innerHTML = ""; // Limpiar el mensaje de error anterior
    }
    ...
    if (errorDni) {
      ...
      document.getElementById(`errorDni${i}`).innerHTML = ""; // Limpiar el mensaje de error anterior
    }
  }